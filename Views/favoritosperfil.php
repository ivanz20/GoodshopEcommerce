...
<div class="container">
    ...
    // print_r($_POST);
    $PublicList = 0;
    $listasUsuario = $servListas->getListByUser($_POST['id_usuario']);
    foreach($listasUsuario  as $results => $row){
        ...
        if($row['privacidad'] == 0){
            $PublicList = $PublicList + 1;
            echo "
        <form method='POST' action='ListaPerfil.php' class='wishlist-card' id='lista" . $row['id_wishlist'] ."' style='width:300px;'>
    ...
    if($PublicList == 0){
        echo "<h2>Este usuario no tiene listas para mostrar publicamente. :(</h2>";
        echo '<img src="https://images.unsplash.com/photo-1617142108319-66c7ab45c711?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxzZWFyY2h8Mnx8c2FkJTIwZG9nfGVufDB8fDB8fA%3D%3D&w=1000&q=80" class="rounded mx-left d-block" alt="perrotruste" width=600px> <br><br>';
    }
